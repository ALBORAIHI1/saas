...
class StripeController extends Controller
{
    ...
    public function vehicleBookingWithStripe(Request $request, $slug, $lang = '')
    {
        if ($lang == '') {
            $lang = 'en';
        }
        ...
        if (isset($this->is_stripe_enabled) && $this->is_stripe_enabled == 'on' && !empty($this->stripe_key) && !empty($this->stripe_secret)) {
            ...
            $request->session()->put('comapany_stripe_data', $data);
            session()->put($request->all(), $slug, $lang);
            $comapany_stripe_data = $comapany_stripe_data ?? false;
            return new RedirectResponse($comapany_stripe_data->url);
        }
    }

    public function vehicleBookingStatus(Request $request, $slug, $lang)
    {
        if ($request->return_type == 'success') {
            $workspace = WorkSpace::where('slug', $slug)->first();
            if (!empty($workspace)) {
                ...
                if ($workspace) {
                    try {
                        if ($request->return_type == 'success') {
                            ...
                            return redirect()->route('vehicle.booking.checkout', [$slug, $payment->booking_id,$lang])->with('success', __('Payment added Successfully'));
                        } else {

                            return redirect()->route('vehicle.booking.manage', [$slug,$lang])->with('error', __('Transaction has been failed!'));
                        }
                    } catch (\Exception $e) {
                        return redirect()->route('vehicle.booking.manage', [$slug,$lang])->with('error', __('Transaction has been failed!'));
                    }
                } else {
                    return redirect()->route('vehicle.booking.manage', [$slug,$lang])->with('error', __('Recipt not found.'));
                }
            } else {
                return redirect()->route('vehicle.booking.manage', [$slug,$lang])->with('error', __('Recipt not found.'));
            }
        } else {
            return redirect()->route('vehicle.booking.manage', [$slug,$lang])->with('error', __('Payment Fail.'));
        }
    }
